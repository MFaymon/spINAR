test_that("spinar output size", {
  ...
  expect_equal(spinar(c(2, 2, 2, 2, 2), 2), c(1, 0, 1, 0, 0))
  expect_equal(spinar(c(2, 2, 2, 2, 2), 1), c(1, 1, 0, 0))
  expect_equal(length(spinar(c(1, 3, 1, 1, 2, 2, 1, 2, 1, 4, 2, 2, 5, 5, 3, 2, 3, 3, 3, 3, 3, 2, 1, 2, 2, 4, 6, 5, 4, 8, 5, 8, 8, 10, 11), 2)), 14)
})
...
test_that("Non-interger p, non-vector dat or missing inputs should error", {
  expect_error(spinar(c(1), 1), "Assertion on 'x' failed: Must have length >= 2, but has length 1.")
  expect_error(spinar(c(1, 0), 2), "Assertion on 'x' failed: Must have length >= 3, but has length 2.")
  expect_error(spinar(c(2, 3, 1, 1, 0), "1"), "Assertion on 'p' failed: Must be of type 'integerish', not 'character'.")
  expect_error(spinar(c(2, 3, 1, 1, 0)), 'argument "p" is missing, with no default')
  expect_error(spinar(c(2, 3), 2), "Assertion on 'x' failed: Must have length >= 3, but has length 2.")
})
