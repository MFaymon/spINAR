...
testthat::test_that("check sum pmf equal 1", {
  expect_equal(sum(package1::spinar(c(2, 0, 1), 1)[2:4]), 1)
  expect_equal(sum(package1::spinar(c(2, 9, 9, 9, 9, 1), 1)[2:11]), 1)
  expect_equal(sum(package1::spinar(c(2, 9, 9, 9, 9, 1), 2)[3:12]), 1)
...
testthat::test_that("Non-interger p, non-vector dat or missing inputs should error", {
  ...
  expect_error(package1::spinar(c(2, 3, 1, 1, 0))) # argument "p" is missing
  expect_error(package1::spinar(c(2, 3), 2)) # no enough big input
  expect_error(package1::spinar(c(2, 2, 2, 2, 2), 2))
