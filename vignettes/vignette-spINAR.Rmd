...
To explore the basic INAR(1), we'll simulate poisson distributions and compare them with the `spinar` function in the `package1` package.
...
The `spinar` function in the `package1` returns the following two parameters:
...
2. The parameters obtained in the `spinar` function are:
  ...
  * The probability mass function denoted by $\pi$, where $\pi =$ `r round(sim_p1[-1],4)` we can see that $\sum_{i=1}^{9} \pi_{i} = $ `r round(sum(sim_p1[-1]),4)`.
...
2. The parameters obtained in the `spinar` function are:
  ...
  * The probability mass function denoted by $\pi$, where $\pi =$ `r round(sim_p2[-(1:2)],4)` we can see that $\sum_{i=1}^12 \pi_{i} = $ `r round(sum(sim_p2[-(1:2)]),4)`.
