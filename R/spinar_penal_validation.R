spinar_penal_wrapper <- function(x, p, validation, penal1, penal2, over, folds = 10, init1 = 1, init2 = 1){
  # x: data
  # p: order of the INAR model
  # validation: TRUE or FALSE depending on whether validation is wanted
  # penal1, penal2: penalization parameter for L1 and L2 penalization resp., will be ignored if validation = TRUE and over = both,
  # for validation = TRUE and over = L1 or L2 only one (resp.) will be ignored; are mandatory if validation = FALSE
  # over: mandatory if validation = TRUE, otherwise it will be ignored, answers whether validation for penal1 (L1) 
  # or penal2 (L2) or both (both) is wanted (error if over is not one of these three)
  # folds: number of folds for cross validation
  # init1 and init2: initial values for penal1 and penal2 in validation
